...
class StatusParser:
    ...
    def __init__(self, file_path=None, stop_event=None):
        self.file_path = file_path if file_path else os.path.join(elite_saved_games_dir(), "Status.json")
    ...
    def _poll_wait(self, seconds=0.5):
        if self.stop_event is not None and self.stop_event.wait(seconds):
